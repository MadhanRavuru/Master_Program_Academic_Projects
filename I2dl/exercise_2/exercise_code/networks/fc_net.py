...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a,fc1_cache = affine_relu_forward(X,W1,b1)
        scores,fc2_cache = affine_forward(a,W2,b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        L2_reg = np.sum(W1*W1) + np.sum(W2*W2)
        loss = loss + 0.5*(self.reg)*L2_reg
       
        dx1,dw2,db2 = affine_backward(dsoftmax,fc2_cache)
        dx,dw1,db1 = affine_relu_backward(dx1,fc1_cache)
